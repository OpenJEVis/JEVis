...
public class CSVImportDialog {
    ...
    private Node buildSeparatorPane() {
        ...
        separatorComboBox.setConverter(new StringConverter<Separator>() {
            ...
            @Override
            public Separator fromString(String string) {
                return separatorComboBox.getItems().stream().filter(separator1 -> separator1.getLocalName().equals(string)).findFirst().orElse(null);
            }
        });
        ...
        enclosedComboBox.setConverter(new StringConverter<Enclosed>() {
            ...
            public Enclosed fromString(String string) {
                return enclosedComboBox.getItems().stream().filter(enclosed -> enclosed.getLocalName().equals(string)).findFirst().orElse(null);
            }
        });
...
//            root.setDisable(true);
            ...
            importTask.setOnSucceeded(event -> {
                ...
                showNotification(importTask.getValue() + " " + I18n.getInstance().getString("csv.import.dialog.success.message"), ControlCenter.getImage("1404237035_Valid.png", 24, 24));
            });
            importTask.setOnFailed(event -> {
                showNotification(importTask.getValue() + " " + I18n.getInstance().getString("csv.import.dialog.failed.message") + " " + importTask.getMessage()
                        , ControlCenter.getImage("1401136217_exclamation-diamond_red.png", 24, 24));
            });
    ...
    private Node buildFileOptions() {
        ...
        charsetBox.setConverter(new StringConverter<Charset>() {
            ...
            public Charset fromString(String string) {
                return charsetBox.getItems().stream().filter(charset1 -> charset1.displayName(I18n.getInstance().getLocale()).equals(string)).findFirst().orElse(null);
            }
        });
...
//        formatOptions = FXCollections.observableArrayList("MS Office, ARA01, Custom");
        ...
        fileButton.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent t) {
                ...
                if (ControlCenter.getLastPath() != null) {
//                    logger.info("Last Path: " + JEConfig.getLastPath().getParentFile());
                    File file = ControlCenter.getLastPath();
                ...
                final File file = fileChooser.showOpenDialog(ControlCenter.getStage());
                if (file != null && file.canRead()) {

                    Platform.runLater(() -> {
                        try {
                            ControlCenter.setLastPath(file);
...
//        }
    ...
    enum Separator {
        ...
        public Separator getSeparator(String localName) {
            final String tabLocalName = I18n.getInstance().getString("csv.seperators.tab");
            final String commaLocalName = I18n.getInstance().getString("csv.seperators.comma");
            final String otherLocalName = I18n.getInstance().getString("csv.seperators.other");
            final String semicolonLocalName = I18n.getInstance().getString("csv.seperators.semi");
            final String spaceLocalName = I18n.getInstance().getString("csv.seperators.space");
            if (localName.equals(tabLocalName)) {
                return Tab;
            } else if (localName.equals(commaLocalName)) {
                return Comma;
            } else if (localName.equals(otherLocalName)) {
                return OTHER;
            } else if (localName.equals(semicolonLocalName)) {
                return Semicolon;
            } else if (localName.equals(spaceLocalName)) {
                return Space;
            }
            throw new IllegalStateException("Unexpected value: " + localName);
        }

        public String getLocalName() {
            return switch (this) {
                case Tab -> I18n.getInstance().getString("csv.seperators.tab");
                case Comma -> I18n.getInstance().getString("csv.seperators.comma");
                case OTHER -> I18n.getInstance().getString("csv.seperators.other");
                case Semicolon -> I18n.getInstance().getString("csv.seperators.semi");
                case Space -> I18n.getInstance().getString("csv.seperators.space");
            };
        }
    }
    ...
    enum Enclosed {
        ...
        public String getLocalName() {
            return switch (this) {
                case Apostrophe -> I18n.getInstance().getString("csv.enclosed.apostrophe");
                case Ditto -> I18n.getInstance().getString("csv.enclosed.ditto");
                case OTHER -> I18n.getInstance().getString("csv.enclosed.other");
                case NONE -> I18n.getInstance().getString("csv.enclosed.none");
                case Gravis -> I18n.getInstance().getString("csv.enclosed.gravis");
            };
        }
    }
