...
public class TimeStampEditor implements AttributeEditor {
    ...
    public DateTime getDateTime() {
        return new DateTime(
                ...
                pickerTime.getLocalTime().getHour(), pickerTime.getLocalTime().getMinute(), pickerTime.getLocalTime().getSecond());
    }
...
//        pickerTime.setConverter(new LocalTimeStringConverter(FormatStyle.MEDIUM));
        ...
        pickerTime.localTimeProperty().addListener((observable, oldValue, newValue) -> {
            if (!newValue.equals(oldValue)) {
                _changed.setValue(Boolean.TRUE);
            }
        });
