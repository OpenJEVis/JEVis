...
import org.jevis.jecc.application.Chart.ChartPluginElements.DataPointTableViewPointer;
...
import org.jevis.jecc.application.Chart.Charts.*;
import org.jevis.jecc.application.Chart.TimeFrame;
import org.jevis.jecc.application.Chart.data.*;
...
//                    "    -fx-text-fill: #0076a3;\n";
    ...
    public void update() {
        ...
        try {
            ...
            ControlCenter.getStatusBar().startProgressJob(ChartPlugin.JOB_NAME, totalJob, I18n.getInstance().getString("plugin.graph.message.startupdate"));
        } catch (Exception ex) {

        }

        ControlCenter.getStatusBar().getTaskList().forEach((task, s) -> {
            if (s.equals(XYChart.class.getName())) {
                task.cancel(true);
            }
        });
...
//                    bp.setCenter(scrollPane);
                ...
                ControlCenter.getStatusBar().progressProgressJob(ChartPlugin.JOB_NAME, 1, I18n.getInstance().getString("plugin.graph.message.finishedchart") + " ");
            }
        }
