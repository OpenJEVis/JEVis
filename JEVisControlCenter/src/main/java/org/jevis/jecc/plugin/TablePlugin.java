...
//        }
    ...
    protected final ToggleButton reduceFractionDigitsButton = new ToggleButton("", ControlCenter.getSVGImage(Icon.LESS_DIGITS, toolBarIconSize, toolBarIconSize, 90));
    protected final ToggleButton increaseFractionDigitsButton = new ToggleButton("", ControlCenter.getSVGImage(Icon.MORE_DIGITS, toolBarIconSize, toolBarIconSize, 90));
    protected final ToggleButton xlsxButton = new ToggleButton("", ControlCenter.getSVGImage(Icon.EXPORT, toolBarIconSize, toolBarIconSize));
                            ...
                            gotoButton.setOnAction(event -> {
                                try {
                                    ...
                                    if (th.isValid() && th.targetObjectAccessible()) {
                                        ...
                                        ControlCenter.openObjectInPlugin(ObjectPlugin.PLUGIN_NAME, findObj);
                                    }
                                } catch (Exception ex) {
                                    logger.error(ex);
                                }
        ...
        button.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                try {
                            ...
                            try {
                                ...
                                if (isPDF) {
                                    Platform.runLater(() -> {
                                        ...
                                        pdfViewerDialog.show(valueChange.getAttribute(), file, ControlCenter.getStage());
                                    });

                                } else {
                                    Platform.runLater(() -> {
                                        ImageViewerDialog imageViewerDialog = new ImageViewerDialog();
                                        imageViewerDialog.show(valueChange.getAttribute(), file, ControlCenter.getStage());
                                    });
                    ...
                    ControlCenter.getStatusBar().addTask(MeterPlugin.PLUGIN_NAME, clearCacheTask, ControlCenter.getImage("measurement_instrument.png"), true);
