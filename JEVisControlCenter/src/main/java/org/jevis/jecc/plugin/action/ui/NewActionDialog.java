...
public class NewActionDialog {
    private static final Logger logger = LogManager.getLogger(NewActionDialog.class);
    ...
    public Response show(Stage owner, JEVisDataSource ds) throws JEVisException {
        JEVisClass actionPlanDirClass = ds.getJEVisClass("Action Plan Directory v2");
        ...
        //TODO JFX17
        comboBox.setCellFactory(new Callback<ListView<JEVisObject>, ListCell<JEVisObject>>() {
            @Override
            public ListCell<JEVisObject> call(ListView<JEVisObject> jeVisObjectListView) {
                return new ListCell<>() {
                    ...
                    @Override
                    protected void updateItem(JEVisObject item, boolean b) {
                        ...
                        try {
                            ...
                            icon.setImage(org.jevis.jecc.tool.ImageConverter.convertToFxImage(actionPlanDirClass.getIcon()));
        ...
        dialog.showAndWait()
                .ifPresent(response -> {
                    if (response.getButtonData().getTypeCode().equals(ButtonType.FINISH.getButtonData().getTypeCode())) {
                        ...
                        NewActionDialog.this.response = Response.YES;
                    } else {
                        NewActionDialog.this.response = Response.CANCEL;
                    }
                });
