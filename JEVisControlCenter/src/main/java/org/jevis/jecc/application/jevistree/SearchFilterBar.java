...
//        this.setBackground(new Background(new BackgroundFill(Color.web("#f4f4f4"), CornerRadii.EMPTY, new Insets(0))));
        ...
        filterBox.setConverter(new StringConverter<JEVisTreeFilter>() {
            ...
            public JEVisTreeFilter fromString(String string) {
                return filterBox.getItems().stream().filter(jeVisTreeFilter -> jeVisTreeFilter.getName().equals(string)).findFirst().orElse(null);
            }
        });
        ...
        replaceMode = ControlCenter.getExpert() && showReplaceBar;
        Platform.runLater(() -> {
