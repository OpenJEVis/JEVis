...
import org.jevis.jecc.application.jevistree.filter.BasicCellFilter;
import org.jevis.jecc.application.jevistree.filter.FilterFactory;
import org.jevis.jecc.application.jevistree.filter.JEVisTreeFilter;
import org.jevis.jecc.application.jevistree.filter.ObjectAttributeFilter;
...
//                    TreeHelper.EventOpenObject(tree, findNode);
                    ...
                    t.consume();
                } else if (deleteAllCleanAndRaw.match(t) && ControlCenter.getExpert()) {
                    TreeHelper.EventDeleteAllCleanAndRaw(tree);
                    t.consume();
                } else if (deleteAllCalculations.match(t) && ControlCenter.getExpert()) {
...
//                    TreeHelper.EventDeleteBrokenTS(tree);
                    ...
                    t.consume();
                } else if (createMultiplierAndDifferential.match(t) && ControlCenter.getExpert()) {
                    TreeHelper.EventCreateMultiplierAndDifferential(tree);
                    t.consume();
                } else if (setLimitsRecursive.match(t) && ControlCenter.getExpert()) {
                    TreeHelper.EventSetLimitsRecursive(tree);
                    t.consume();
                } else if (setSubstitutionSettingsRecursive.match(t) && ControlCenter.getExpert()) {
                    TreeHelper.EventSetSubstitutionSettingsRecursive(tree);
                    t.consume();
                } else if (setUnitAndPeriod.match(t) && ControlCenter.getExpert()) {
                    TreeHelper.EventSetUnitAndPeriodRecursive(tree);
                    t.consume();
                } else if ((enableAll.match(t) || disableAll.match(t)) && ControlCenter.getExpert()) {
