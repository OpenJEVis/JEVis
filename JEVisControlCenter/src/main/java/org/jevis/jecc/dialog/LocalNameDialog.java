    ...
    private MFXComboBox<Locale> buildLangBox(Locale selected) {
        ...
        picker.setConverter(new StringConverter<Locale>() {
            ...
            public String toString(Locale object) {
                if (object != null) {
                    return object.getDisplayName(I18n.getInstance().getLocale());
                } else return "";
            }

            @Override
            public Locale fromString(String string) {
                return picker.getItems().stream().filter(locale -> locale.getDisplayName(I18n.getInstance().getLocale()).equals(string)).findFirst().orElse(null);
            }
        });
