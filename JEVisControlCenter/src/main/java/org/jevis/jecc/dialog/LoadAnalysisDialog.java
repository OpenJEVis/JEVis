...
public class LoadAnalysisDialog extends Dialog {
    ...
    private void initializeControls() {
        ...
        loadButton.setOnAction(actionEvent -> {
            ...
            if (analysisTimeFrame == null && comboBoxCustomPeriods.getSelectionModel().getSelectedIndex() > 0) {
            ...
            } else if (analysisTimeFrame != null && analysisTimeFrame.getTimeFrame().equals(TimeFrame.CUSTOM)) {
                DateTime start = new DateTime(pickerDateStart.getValue().getYear(), pickerDateStart.getValue().getMonthValue(), pickerDateStart.getValue().getDayOfMonth(),
                        pickerTimeStart.getLocalTime().getHour(), pickerTimeStart.getLocalTime().getMinute(), pickerTimeStart.getLocalTime().getSecond());
                DateTime end = new DateTime(pickerDateEnd.getValue().getYear(), pickerDateEnd.getValue().getMonthValue(), pickerDateEnd.getValue().getDayOfMonth(),
                        pickerTimeEnd.getLocalTime().getHour(), pickerTimeEnd.getLocalTime().getMinute(), pickerTimeEnd.getLocalTime().getSecond());
    ...
    private DisabledItemsComboBox<ManipulationMode> getMathBox() {
        ...
        math.setConverter(new StringConverter<ManipulationMode>() {
            ...
            public ManipulationMode fromString(String string) {
                return math.getItems().stream().filter(manipulationMode -> ManipulationMode.getListNamesManipulationModes().get(ManipulationMode.parseManipulationIndex(manipulationMode)).equals(string)).findFirst().orElse(null);
            }
        });
    ...
    private void addListener() {
        ...
        pickerTimeStart.localTimeProperty().addListener((observable, oldValue, newValue) -> {
            if (newValue != null && newValue != oldValue) {
                DateTime start = chartPlugin.getDataSettings().getAnalysisTimeFrame().getStart();
                DateTime end = chartPlugin.getDataSettings().getAnalysisTimeFrame().getEnd();
                DateTime now = DateTime.now();
        ...
        pickerTimeEnd.localTimeProperty().addListener((observable, oldValue, newValue) -> {
            if (newValue != null && newValue != oldValue) {
                DateTime start = chartPlugin.getDataSettings().getAnalysisTimeFrame().getStart();
                DateTime end = chartPlugin.getDataSettings().getAnalysisTimeFrame().getEnd();
                DateTime now = DateTime.now();
