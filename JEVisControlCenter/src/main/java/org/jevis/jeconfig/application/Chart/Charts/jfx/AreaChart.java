...
package org.jevis.jeconfig.application.Chart.Charts.jfx;
...
public class AreaChart<X, Y> extends XYChart<X, Y> {
  ...
  static <X, Y> void makePaths(XYChart<X, Y> chart, Series<X, Y> series,
                               ...
                               double yAnimMultiplier, LineChart.SortingPolicy sortAxis) {
    final Axis<X> axisX = chart.getXAxis();
    final Axis<Y> axisY = chart.getYAxis();
    final double hlw = linePath.getStrokeWidth() / 2.0;
    final boolean sortX = (sortAxis == LineChart.SortingPolicy.X_AXIS);
    final boolean sortY = (sortAxis == LineChart.SortingPolicy.Y_AXIS);
  ...
  protected void layoutPlotChildren() {
    ...
    for (int seriesIndex = 0; seriesIndex < getDataSize(); seriesIndex++) {
      ...
      makePaths(this, series, constructedPath, fillPath, linePath,
              seriesYAnimMultiplier.get(), LineChart.SortingPolicy.X_AXIS);
    }
  }
