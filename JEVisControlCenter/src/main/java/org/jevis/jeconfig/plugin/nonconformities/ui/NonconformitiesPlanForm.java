...
public class NonconformitiesPlanForm extends Alert {
    ...
    public NonconformitiesPlanForm(Nonconformities nonconformities) {
        ...
        updateView(nonconformities);
    }


    public void updateView(Nonconformities nonconformities) {
        //nameField.setText(actionPlan.getName());

        nameField.textProperty().bindBidirectional(nonconformities.getName());
        System.out.println("Init status: " + nonconformities.getStatustags());
        statusListView.setItems(nonconformities.getStatustags());
        fieldsListView.setItems(nonconformities.getFieldsTags());
        mediumListView.setItems(nonconformities.getMediumTags());
    }
    ...
    private GridPane buildENPIList(ListView<JEVisObject> listView) {
        ...
        enpiListView.setItems(nonconformities.getEnpis());
        ...
        addButton.setOnAction(event -> {
            try {
                ...
                TreeSelectionDialog treeSelectionDialog = new TreeSelectionDialog(nonconformities.getObject().getDataSource(), classes, SelectionMode.MULTIPLE, new ArrayList<>(), false);
                ...
                if (optional.get() == treeSelectionDialog.buttonOK) {
                    ...
                    treeSelectionDialog.getUserSelection().forEach(userSelection -> {
                        nonconformities.getEnpis().add(userSelection.getSelectedObject());
                    });
                }
