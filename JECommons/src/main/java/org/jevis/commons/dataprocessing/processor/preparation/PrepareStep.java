...
//        if (rawSamplesDown.isEmpty() || rawSamplesUp.isEmpty()) {
    ...
    private List<CleanInterval> getIntervals(CleanDataObject calcAttribute, List<PeriodRule> periodCleanData, List<PeriodRule> periodRawData) throws JEVisException {
...
//            logger.error("Cant calculate the intervals with start date " + datePattern.print(currentDate) + " and end date " + datePattern.print(maxEndDate));
            ...
            Period firstPeriod = CleanDataObject.getPeriodForDate(periodCleanData, currentDate);

            boolean periodCleanHasYear = firstPeriod.getYears() > 0;
            boolean periodCleanHasMonths = firstPeriod.getMonths() > 0;
            boolean periodCleanHasWeeks = firstPeriod.getWeeks() > 0;
            boolean periodCleanHasDays = firstPeriod.getDays() > 0;
            boolean periodCleanHasHours = firstPeriod.getHours() > 0;
            boolean periodCleanHasMinutes = firstPeriod.getMinutes() > 0;
            boolean periodCleanHasSeconds = firstPeriod.getSeconds() > 0;
            ...
            while (currentDate.isBefore(maxEndDate) && !periodCleanData.isEmpty() && !currentDate.equals(lastDate)) {
                ...
                Period period = CleanDataObject.getPeriodForDate(periodCleanData, currentDate);
                ...
                if (periodCleanHasYear) {
                    startInterval = startInterval.minusYears(period.getYears()).withMonthOfYear(1).withDayOfMonth(1);
                    endInterval = startInterval.plusYears(period.getYears()).withMonthOfYear(1).withDayOfMonth(1).minusDays(1);
                }
                if (periodCleanHasMonths) {
                    startInterval = startInterval.minusMonths(period.getMonths()).withDayOfMonth(1);
                    endInterval = startInterval.plusMonths(period.getMonths()).withDayOfMonth(1).minusDays(1);
                }
                if (periodCleanHasWeeks) {
                    startInterval = startInterval.minusWeeks(period.getWeeks()).withDayOfWeek(1);
                    endInterval = startInterval.plusWeeks(period.getWeeks()).withDayOfWeek(1).minusDays(1);
                }
                if (periodCleanHasDays) {
                    startInterval = startInterval.minusDays(period.getDays());
                }
                if (periodCleanHasHours) {
                    startInterval = startInterval.minusHours(period.getHours());
                }
                if (periodCleanHasMinutes) {
                    startInterval = startInterval.minusMinutes(period.getMinutes());
                }
                if (periodCleanHasSeconds) {
                    startInterval = startInterval.minusSeconds(period.getSeconds());
                }
    ...
    private List<CleanInterval> getIntervalsFromRawSamples(CleanDataObject calcAttribute, List<JEVisSample> rawSamples) throws Exception {
        ...
        for (JEVisSample curSample : rawSamples) {
            ...
            Period periodForDate = CleanDataObject.getPeriodForDate(calcAttribute.getRawDataPeriodAlignment(), timestamp);
