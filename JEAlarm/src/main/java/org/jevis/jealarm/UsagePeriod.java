...
public class UsagePeriod {
    ...
    private int _times;
    private int _days;
    private final UsagePeriodType _type;

    public UsagePeriod(UsagePeriodType type) {
        _type = type;
    }

    public UsagePeriodType getType() {
        return _type;
    }
    ...
    private int getPeriod(JEVisObject alarmObj, String attrName) {
        try {
            ...
            try {
                ...
                logger.info("period is empty. default value added", ex);
                return NODATA;
            }
        } catch (Exception ex) {
    ...
    private boolean isValueSelected(int val, int n) {
        int bitmask = 1;
        bitmask = bitmask << (val);
        logger.debug(Integer.toBinaryString(bitmask));
        return ((n & bitmask) != 0);
    ...
    public void setPeriod(Long silentTime) {
        _times = safeLongToInt(silentTime);
