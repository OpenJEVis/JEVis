...
public class LoytecXmlDlChannelDirectory implements LoytecXmlDlChannelDirectoryClass {
    ...
    private List<UPCChannel> channels = new ArrayList<>();
    ...
    public LoytecXmlDlChannelDirectory(JEVisObject channelDirectoryObject) {
        ...
        try {
            ...
            listChannels.forEach(channelObject -> {
                try {
                    JEVisAttribute attTargetId = channelObject.getAttribute(UPCChannel.TARGET_ID);
                    JEVisAttribute attTrendId = channelObject.getAttribute(UPCChannel.TREND_ID);
//                    JEVisAttribute attLastReadOut = channelObject.getAttribute(UPCChannel.LAST_READOUT);
//                    if (attTargetId.hasSample() && attTrendId.hasSample() && attLastReadOut.hasSample()) {
                    if (attTargetId.hasSample() && attTrendId.hasSample()) {
                        if (!counterCheckForErrorInAPI.contains(channelObject.getID())) {
                            channels.add(new UPCChannel(channelObject));
    ...
    @Override
    public List<UPCChannel> getChannels() {
        return channels;
    }
}
